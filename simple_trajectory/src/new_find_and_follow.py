...
def get_pose_ref():
    x_line_1 = 0#line_point_1_cb.x
    y_line_1 = 4#line_point_1_cb.y
    z_line_1 = 1.5#line_point_1_cb.z

    x_line_2 = 3#line_point_2_cb.x
    y_line_2 = 2#line_point_2_cb.y
    z_line_2 = 1.5#line_point_2_cb.z

    x_ref = 0#get_trajectory.x
    y_ref = 2.5#get_trajectory.y
    z_ref = 1.5#get_trajectory.z

    v_y = y_line_2 - y_line_1 # y segment
    v_x = x_line_1- x_line_2 # x segment
    v_z = z_line_1 - z_line_2 # z segment = 0
    ...
    v.position.x = -v_x * rate  
    v.position.y = v_y * rate 
    v.position.z = 1 * rate 

    pose_ref.position.x = x_ref + v.position.x #msg.position.x + v.position.x 
    pose_ref.position.y = y_ref + v.position.y #msg.position.y + v.position.y
    pose_ref.position.z = z_ref + v.position.z #msg.position.z + v.position.z

    # TODO: z_ref
    if pose_ref.position.z >= z_ref:
        pose_ref.position.z = z_ref

    x_ref = pose_ref.position #msg.position
...
def talker():
    ...
    while not rospy.is_shutdown():
        if uav_pose_cb.p.y >= y_konacno - 0.3:
            flag = 1
