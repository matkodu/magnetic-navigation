...
void GazeboWindPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  if (_sdf->HasElement("xyzOffset"))
    xyz_offset_ = _sdf->GetElement("xyzOffset")->Get<ignition::math::Vector3<double>>();
  else
  ...
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "windDirection", wind_direction_, wind_direction_);
  ...
  getSdfParam<ignition::math::Vector3<double>>(_sdf, "windGustDirection", wind_gust_direction_, wind_gust_direction_);
...
void GazeboWindPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  ignition::math::Vector3<double> wind = wind_strength * wind_direction_;
  // Apply a force from the constant wind to the link.
  link_->AddForceAtRelativePosition(wind, xyz_offset_);

  ignition::math::Vector3<double> wind_gust(0, 0, 0);
